...
- **Auth/session endpoints** need captured credentials, and those expire. The derivation output shows sensitive header/query names and JSON field shapes but replaces their values with `<redacted>`; obtain required values directly from the private HAR when building the client. Both capturers force HAR mode `0600`, but HARs still contain live secrets and response data — keep them out of logs/repos and delete them after deriving.
- **Embedded bodies make big HARs.** Local Playwright capture embeds full bodies. CDP capture skips/truncates individual response bodies above 1 MB; use `--max-body` to further cap what the derivation prints.
