...
- **Tests must be hermetic.** Do not write tests that touch the real `~/.hermes/`. Use `scripts/run_tests.sh`, not direct `pytest`, for CI-parity.
...
- Do not use ANSI erase-to-EOL (`[K`) in spinner/display code; use space padding.
