...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    p_complete.add_argument("--verdict", choices=sorted(kb.VALID_COMPLETION_VERDICTS),
                            help="Semantic review verdict. BLOCK/FAIL keep the task blocked; PASS marks it done.")
    ...
    with kb.connect_closing() as conn:
        for tid in ids:
            ...
            if not kb.complete_task(
                ...
                if completed_task and completed_task.status == "blocked":
                    run = kb.latest_run(conn, tid)
                    semantic_verdict = (
                        (run.metadata or {}).get("verdict")
                        if run and isinstance(run.metadata, dict)
                        else None
                    )
                    print(
                        f"Recorded {semantic_verdict or 'blocking'} verdict for {tid}; "
