...
class SystemdOperations:
    ...
    def _assert_drain_owned(manifest: Manifest) -> None:
        ...
        if not isinstance(marker, dict) or marker.get("owner_token") != manifest.transaction_id:
            raise ControllerBlocked("draining gateway is not owned by this transaction")
    ...
    def sample_occupancy(self, manifest: Manifest) -> Occupancy:
        ...
        if manifest.bootstrap_mode == _BOOTSTRAP_MODE:
            ...
            if (
                state.get("gateway_state") not in {"running", "draining"}
                or reported_total < 0
            ):
