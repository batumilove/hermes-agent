...
def test_watchdog_snapshot_caps_units_and_preserves_aggregate_counts(
    tmp_path: Path, monkeypatch
):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    runner, _adapter = make_restart_runner()
    runner._running_agents = {
        f"agent:main:test:{idx}": _Agent(f"session-{idx}") for idx in range(300)
    }
    runner._running_agents_ts = {
        key: float(idx) for idx, key in enumerate(runner._running_agents)
    }
    runner._session_run_generation = {key: 1 for key in runner._running_agents}

    snapshot = GatewayRunner._shutdown_work_attribution_snapshot(runner)
    recorder = DrainAttributionRecorder(
        home=tmp_path,
        generation=current_drain_generation(),
        owner_probe=lambda _generation: True,
        max_units=256,
    )
    result = recorder.record(phase="forced_forward", **snapshot)
    payload = json.loads(result.path.read_text(encoding="utf-8").splitlines()[0])

    assert payload["counts"] == {"agent": 300, "cron": 0, "api": 0, "total": 300}
    assert len(payload["units"]) == 256
    assert payload["attribution_complete"] is False
    assert payload["omissions"] == ["units_truncated:44"]
