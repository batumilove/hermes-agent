...
class TestChangedValuesAlwaysPersist:
    def test_update_session_uses_one_upsert_not_full_replacement(
        self, tmp_path, monkeypatch
    ):
        store = _make_store(tmp_path, monkeypatch)
        entry = store.get_or_create_session(_source())
        operations = []
        real_upsert = store._db.save_gateway_routing_entry

        def record_upsert(*args, **kwargs):
            operations.append("save_gateway_routing_entry")
            return real_upsert(*args, **kwargs)

        monkeypatch.setattr(store._db, "save_gateway_routing_entry", record_upsert)
        monkeypatch.setattr(
            store._db,
        ...
        assert operations == ["save_gateway_routing_entry"]
        store._db.close()

    def test_healthy_lookup_uses_one_upsert_not_full_replacement(
        self, tmp_path, monkeypatch
    ):
        store = _make_store(tmp_path, monkeypatch)
...
class TestGenerationOrdering:
    def test_full_and_fast_saves_share_one_monotonic_counter(
        self, tmp_path, monkeypatch
    ):
        store = _make_store(tmp_path, monkeypatch)
        entry = store.get_or_create_session(_source())
        starting_revision = store._routing_generation

        with store._lock:
            _, full_revision = store._snapshot_routing_locked()
        store._save_entry(entry.session_key)
        fast_revision = store._fast_persisted_entries[entry.session_key][0]

        assert full_revision == starting_revision + 1
        assert fast_revision == full_revision + 1
        assert store._routing_generation == fast_revision
        store._db.close()
