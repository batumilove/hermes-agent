...
def test_main_pool_id_match_preserves_failed_bearer_after_peer_rotation(monkeypatch):
    """Stable entry IDs must not replace the bearer identity from the failed request."""
    failed_token = _invoke_jwt(seconds=3000)
    peer_token = _invoke_jwt(seconds=3600)
    ...
    def _resolve(**kwargs):
        seen.update(kwargs)
        return {"api_key": peer_token}

    monkeypatch.setattr(auth_mod, "resolve_nous_runtime_credentials", _resolve)
    ...
    assert seen["stale_access_token"] == failed_token
