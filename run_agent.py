...
class AIAgent:
        ...
        if not _is_subagent:
            try:
                ...
                if payload.get("status") == "dispatched" and delegation_id:
                    detached = getattr(self, "_turn_detached_work", None)
                    if not isinstance(detached, list):
                        detached = []
                        self._turn_detached_work = detached
                    detached.append({
                        "kind": "delegation",
                        "id": delegation_id,
                    })
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        self._record_detached_tool_result(function_name, function_args, result)
        return result

    def _record_detached_tool_result(
        self,
        function_name: str,
        function_args: dict,
        result: Any,
    ) -> None:
        """Record successfully-started detached work for the current turn.

        Concurrent tools pass through :meth:`_invoke_tool`; the normal
        single-call production path dispatches directly from
        ``agent.tool_executor`` and calls this helper after execution.
        """
        if function_name == "terminal" and function_args.get("background") is True:
            try:
                payload = json.loads(result)
                process_id = str(payload.get("session_id") or "")
                if process_id and not payload.get("error"):
                    detached = getattr(self, "_turn_detached_work", None)
                    if not isinstance(detached, list):
                        detached = []
                        self._turn_detached_work = detached
                    detached.append({"kind": "process", "id": process_id})
            except (TypeError, ValueError, json.JSONDecodeError):
                pass
