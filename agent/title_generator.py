...
def auto_title_session(
    ...
    Silently skips if:
    - session_db is None
    - the first-title path finds a title already set
...
def _auto_title_session(
    ...
    try:
        if update_existing:
            try:
                persisted = title if session_db.set_session_title(session_id, title) else None
            except ValueError:
                next_title_fn = getattr(session_db, "get_next_title_in_lineage", None)
                if next_title_fn is None:
                    raise
                ...
                if not deduped or deduped == title:
                    raise
                persisted = deduped if session_db.set_session_title(session_id, deduped) else None
        else:
