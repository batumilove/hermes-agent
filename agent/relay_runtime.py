...
class RelayRuntime:
    ...
    def _close_session(self, event: dict[str, Any]) -> None:
        ...
        if session is None:
            return
        self._flush_subscribers_bounded(failures)
        self._finish_close_session(session_id, session, failures)

    def _flush_subscribers_bounded(self, failures: list[str]) -> None:
        """Flush subscribers off the caller's loop, bounded, without holding the
        host operation gate (a process-wide flush may wait on publications
        owned by other sessions; final plugin teardown flushes once more after
        all tracked operations drain)."""
        try:
            try:
            ...
            if not had_active_turn:
                try:
                    ...
                    if (config["on_compaction"] and session.rotate_pending) or (
                        ...
                        host.rotate_session_scope(session, reason=reason)
                except Exception:
                    logger.warning(
