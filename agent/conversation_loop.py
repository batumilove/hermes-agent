...
def run_conversation(
    ...
    agent._context_efficiency_user_message = _summarize_user_message_for_log(
    ...
    # Operation-scoped detached-work ledger. Tool dispatchers append only work
    # accepted during THIS user turn; the gateway uses it to distinguish a
    # dispatch acknowledgement from actual task completion.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                # A Codex stream that connects but emits no bytes/events is a
                # model/backend rejection signal, not a useful transient retry.
                # Switch to the configured fallback immediately and preserve the
                # structured reason for observability and tests.
                if classified.reason == FailoverReason.silent_hang:
                    if agent._has_pending_fallback():
                        agent._buffer_status(
                            "⚠️ Codex stream stalled before first output — switching to fallback..."
                        )
