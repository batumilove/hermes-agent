...
def _run_review_in_thread(
    ...
    try:
        ...
        with thread_scoped_silence():
            ...
            # uses to proactively cancel a still-running review; upstream now
            # additionally fences startup and request-phase exit with a
            # per-review run token, which remains the live-turn cancellation
            # authority). Without tracking, a review still streaming when the
            # next turn starts races the live turn against the same
            # session_id/credentials — producing doubled prompt-token accounting
            # and a Ctrl+C-proof lockup. Best-effort: agents built without
            # agent_init.py (test stubs) degrade to "no cross-cancellation"
