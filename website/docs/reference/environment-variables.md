...
| `HERMES_KANBAN_NOTIFY_IN_GATEWAY` | Runtime override for `kanban.notify_in_gateway`. Set to `0`, `false`, `no`, or `off` to keep the gateway from polling Kanban subscriptions and sending task event notifications. Defaults to enabled, including when dispatch is external. |
